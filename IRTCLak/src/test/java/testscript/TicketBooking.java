	...
	List<WebElement> optionsfrom = wait.until(
		ExpectedConditions.presenceOfAllElementsLocatedBy(By.xpath("//div[@role='option']") ));
				...
				for (WebElement opt : optionsfrom) {
				    if (opt.getText().trim().equalsIgnoreCase("Chetlat")) ////Kochi,Kavaratti,Beypore,Mangalore,Androth,Kalpeni,Agatti,Kalpeni,Bangaram,Minicoy,Amini,Kadmat,Kiltan,Chetlat,Bitra
				    {
				        opt.click();
				        break;
				    }
				...
				List<WebElement> optionsTo = wait.until(
					...
					for (WebElement opt : optionsTo) {
					    if (opt.getText().trim().equalsIgnoreCase("Kiltan"))//Kochi,Kavaratti,Beypore,Mangalore,Androth,Kalpeni,Agatti,Kalpeni,Bangaram,Minicoy,Amini,Kadmat,Kiltan,Chetlat,Bitra
					    {
					        opt.click();
					        break;
					    }
					 ...
					 datelabel.click();
					 WebElement date=wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//div[@class='react-datepicker__day react-datepicker__day--023 react-datepicker__day--highlighted']")));
					 date.click();
					 
					 
					 LocalTime targetTime = LocalTime.of(20, 8);
